...
, dc.elias_id
, dc.espn_id
from read_parquet('${bucket}/bronze/depth_charts/season=*/depth_charts.parquet', union_by_name = true) dc
	left outer join read_csv('${bucket}/mappings/club_mapping.csv') cm_cc
	  on dc.club_code = cm_cc.source
	left outer join read_csv('${bucket}/mappings/club_mapping.csv') cm_t
	  on dc.team = cm_t.source
	left outer join read_parquet('${bucket}/silver/schedules/schedules.parquet') s
	  on dc.season = s.season
	  and dc.week = s.week
	  and (COALESCE(cm_t.target, cm_cc.target) = s.away_team or COALESCE(cm_t.target, cm_cc.target) = s.home_team)
	left outer join read_csv('${bucket}/mappings/formation_mapping.csv') fm
