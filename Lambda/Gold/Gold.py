...
Generic gold-layer runner: reads a manifest describing which SQL queries
...
DynamoDB. Adding a new gold transform is just adding a new .sql file + a
manifest entry - no code change or redeploy needed for that. Code changes
are only needed for genuinely new *logic* that can't be expressed as a
plain SELECT.

Structurally this is the same runner as Lambda/Silver/Silver.py - manifest
format, DuckDB execution, async entry/worker split, and job tracking are
all identical. The one real difference is semantic, not code: silver
queries are restricted to per-source technical cleansing (no cross-source
joins); gold queries are exactly where those joins happen (e.g. joining
silver's rosters/weekly_stats/depth_charts/etc. into a unified
player_week table on gsis_id+season+week), plus the engineered features on
top (recency-weighted PPG, situation-change detection, volatility scoring,
draft score) - see docs/project-summary.md for the full gold-layer design
and issue #27 for what's actually scoped/built so far. The manifest here
starts empty - no gold queries exist yet, this is just the execution
scaffold for when that design work lands.

Manifest and query files live in S3 (uploaded via the same CI/CD step that
uploads the mapping CSVs and silver queries - see Lambda/Gold/queries/ in
the repo, and .github/workflows/deploy-FantasyFootballManager.yml), at:
    s3://<bucket>/gold/queries/manifest.json
    s3://<bucket>/gold/queries/<query_file>

Manifest shape (see Lambda/Gold/queries/manifest.json for the real one):
    [
      {"name": "player_week", "query_file": "player_week.sql", "output_path": "gold/player_week/player_week.parquet"},
      ...
    ]
...
silver's for the bucket URI, substituted at runtime so the same files
work unchanged across dev/prod. `${bucket}` (not a bare token like
...
entry fails, the others still run (matching Silver/BronzeBackfill's
per-dataset error isolation), and the job's `errors` list records which
entries failed and why.

Async entry/worker split, same pattern and same reason as Silver.py and
...
Lambda itself. The entry path validates + async self-invokes a worker +
returns 202 immediately; the worker (detached, no API Gateway involved)
does the actual multi-query run and updates the same DynamoDB job record.

Deploy notes (console-first, matching Silver.py's pattern):
    - Runtime: Python 3.12 specifically - NOT 3.14. duckdb doesn't yet
      publish cp314 wheels (confirmed via chat), only up to 3.12 at time of
      writing. Needs the `duckdb` package in a layer - same layer Silver
      uses can likely be reused as-is.
    - Execution role: needs s3:ListBucket (with s3:prefix condition
      covering "silver/*", "mappings/*", and "gold/queries/*") on the
      bucket, PLUS s3:GetObject on those same three prefixes, PLUS
      s3:PutObject on "gold/*", PLUS dynamodb:PutItem/UpdateItem scoped
    ...
    - API Gateway routes, both with the Cognito JWT authorizer attached,
      both Lambda proxy integration, both pointing at this function:
        POST /admin/gold
        GET  /admin/gold/status/{jobId}
    - Environment variables LC_ALL=C.UTF-8, LANG=C.UTF-8, PYTHONUTF8=1 -
      same reason as Silver.py: Lambda's minimal environment has no locale
      set, which DuckDB/Python consult directly, causing
      UnicodeDecodeError on non-ASCII data if unset.
...
JOB_TYPE = "gold"
ADMIN_GROUP = "admin"
MANIFEST_KEY = "gold/queries/manifest.json"
QUERIES_PREFIX = "gold/queries/"
...
def _is_admin(event: dict) -> bool:
    # Same claim-parsing logic as BronzeBackfill.py/Silver.py - see those
...
def _finish_job(job_id: str, status: str, written: list[str], errors: list[str]) -> None:
    message = f"Gold run complete: {len(written)} output(s) written" + (
...
def _handle_worker(event: dict) -> None:
    ...
    try:
        ...
        # icu provides real IANA timezone data - same rationale as Silver.py
        # (schedules.sql's gameday/gametime -> UTC standardization); kept
        # here too since gold queries will join against silver's schedules
        # output and may need the same timezone handling.
        ...
        # forever.
        log.exception("Gold run crashed before any entry could run")
        _finish_job(job_id, "failed", [], [str(e)])
        return

    # "success" even with partial errors, matching Silver/BronzeBackfill's
...
def _handle_start_gold(context) -> dict:
...
def handler(event, context):
    ...
    return _handle_start_gold(context)
